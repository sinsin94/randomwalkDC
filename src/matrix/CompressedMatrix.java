...
public class CompressedMatrix {
	 ...
	 *********************
	 */
	public CompressedMatrix(String paraFilename, double paraDC,int paraK) {
		data = null;
		try {
			FileReader fileReader = new FileReader(paraFilename);
			data = new Instances(fileReader);
		...
		// record real label
		reallable = new int[data.numInstances()];
		for (int i = 0; i < data.numInstances(); i++) {
			reallable[i] = (int) data.instance(i).classValue();
		}

		// kNearestNeighbors(0, 3);
		// Scan the data to construct the matrix
		// record the density
		density = new int[data.numInstances()];
		Arrays.fill(density, -1);
		for (int i = 0; i < matrix.length; i++) {
			matrix[i] = kNearestNeighbors(i, paraDC, paraK);
		} // Of for i

		System.out.println(this);
		System.out.println("The density is :" + Arrays.toString(density));
		symmetrize();
	 ...
	 *********************
	 */
	Triple kNearestNeighbors(int paraNode, double paraDC,int paraK) {
...
//	Triple kNearestNeighbors(int paraNode, double paraDc) {
//		Triple resultHeader = new Triple();
//		Triple tempReference = resultHeader;
//
//		double tempSimilarity = 0;
...
//				density[i] ++;
//				Triple tempNewTriple = new Triple();
//				tempNewTriple.column = i;
//				tempNewTriple.weight = tempSimilarity;
//				// Link it now!
//				tempReference.next = tempNewTriple;
//				tempReference = tempNewTriple;
//				
...
//	}// Of kNearestNeighbors
	...
	/*****
	 * Compute the Overlap distance between each pair of instances.distanceԽСԽ���� For
	 * symbolic data.
	 */
	protected double computeOverlapDistance(int paraI, int paraJ) {
		int distance = 0;
		for (int i = 0; i < data.numAttributes() - 1; i++) {
			// get the value of attribute
			double valueA = data.instance(paraI).value(i);
			double valueB = data.instance(paraJ).value(i);
			// match the values
			if (valueA != valueB) {
				distance++;
			} // Of if
	...
	public static CompressedMatrix multiply(CompressedMatrix paraMatrix1, CompressedMatrix paraMatrix2) {
		...
		OrderedIntArray tempArray = new OrderedIntArray(100000);
	...
	public int[] depthFirstClustering(double paraCutThreshold) {
		...
		System.out.println("The clustering depthFirstClustering result is: " + Arrays.toString(clusterNumbers));
	...
	public int[] widthFirstClustering(double paraCutThreshold) throws Exception {
		...
		System.out.println("The clustering widthFirstClustering result is: " + Arrays.toString(clusterNumbers));
	...
	public static void main(String args[]) {
		...
		CompressedMatrix tempMatrix = new CompressedMatrix(
				"/Users/dengsiyu/eclipse/workspace/randomwalk/data/example21.arff");
